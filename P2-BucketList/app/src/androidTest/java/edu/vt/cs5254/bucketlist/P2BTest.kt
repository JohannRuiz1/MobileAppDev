...
// This Regular Expression pattern matches the timestamp of all the pre-populated goals.
// This pattern accounts for most (but not all) time zones around the world.
// Please ask for guidance in Piazza if your time zone isn't supported.
private const val DB_TIMESTAMP_PATTERN = "Last updated 2025-02-25 at ..:[25]5:02 [AP]M"
...
class P2BTest {
    ...
    fun goalFourCheckDetails() {
        ...
        onView(withId(R.id.last_updated_text))
            .check(matches(withDbTimestampPattern()))
        checkNoteGone(0)
    ...
    fun goalFourRotateClickPauseThenBackRotateAndConfirmList() {
        ...
        checkNoteGone(1)
        onView(withId(R.id.last_updated_text))
            .check(matches(not(withDbTimestampPattern())))
        pressBack()
        scenario.recreate()
        onView(withId(R.id.goal_recycler_view))
            .check(
    ...
    fun goalEightUncheckPausedCheckCompletedConfirmLastUpdated() {
        ...
        sleep(1000) // Wait for the time to advance to the next second
        changeCheckboxState(CHECKBOX.COMPLETED, true)
    ...
    private fun withDbTimestampPattern(regex: String = DB_TIMESTAMP_PATTERN): Matcher<View?> {
